...
CLOUD.ViewHouse = function (viewer) {
    ...
    var createHouse = function () {
        ...
        var namesList = [
            _componentNames.BottomSouth, _componentNames.BottomSouthEast, _componentNames.BottomEast, _componentNames.BottomNorthEast,
            _componentNames.BottomNorth, _componentNames.BottomNorthWest, _componentNames.BottomWest, _componentNames.BottomSouthWest,
            _componentNames.BottomFloor,
            _componentNames.MiddleSouth, _componentNames.MiddleSouthEast, _componentNames.MiddleEast, _componentNames.MiddleNorthEast,
            _componentNames.MiddleNorth, _componentNames.MiddleNorthWest, _componentNames.MiddleWest, _componentNames.MiddleSouthWest,
            _componentNames.MiddleDoor, _componentNames.MiddleRightWindow, _componentNames.MiddleLeftWindow,
            _componentNames.RoofSouth, _componentNames.RoofSouthEast, _componentNames.RoofEast, _componentNames.RoofNorthEast,
            _componentNames.RoofNorth, _componentNames.RoofNorthWest, _componentNames.RoofWest, _componentNames.RoofSouthWest,
            _componentNames.RoofCenter, _componentNames.RoofEaves
        ];
    ...
    var switchView = function (name) {
        ...
        switch (name) {
            ...
            case _componentNames.RoofCenter:
                if (_currentViewMode === _enumViewMode.RoofFront ||
                    _currentViewMode === _enumViewMode.RoofSouthEast ||
                    _currentViewMode === _enumViewMode.RoofSouthWest) {
                    viewMode = _enumViewMode.Top;
                } else if (_currentViewMode === _enumViewMode.RoofRight) {
                    viewMode = _enumViewMode.TopTurnRight;
                } else if (_currentViewMode === _enumViewMode.RoofBack ||
                    _currentViewMode === _enumViewMode.RoofNorthEast ||
                    _currentViewMode === _enumViewMode.RoofNorthWest) {
            ...
            case _componentNames.BottomFloor:
                if (_currentViewMode === _enumViewMode.BottomFront ||
                    _currentViewMode === _enumViewMode.BottomSouthEast ||
                    _currentViewMode === _enumViewMode.BottomSouthWest) {
                    viewMode = _enumViewMode.Bottom;
                } else if (_currentViewMode === _enumViewMode.BottomRight) {
                    viewMode = _enumViewMode.BottomTurnLeft;
                } else if (_currentViewMode === _enumViewMode.BottomBack ||
                    _currentViewMode === _enumViewMode.BottomNorthEast ||
                    _currentViewMode === _enumViewMode.BottomNorthWest) {
            ...
            case _componentNames.RoofNorthEast:
                ...
                break;
            case _componentNames.ControlPointNorth: // 前 - 顶 - 后 - 后顶 - 前
                ...
                break;
            case _componentNames.ControlPointSouth: // 前 - 底 - 后 - 后底 - 前
                if (_currentViewMode === _enumViewMode.Front) {
                    viewMode = _enumViewMode.Bottom;
                ...
                break;
            case _componentNames.ControlPointEast: // 前 - 右 - 后 - 左 - 前
                if (_currentViewMode === _enumViewMode.Front) {
                    viewMode = _enumViewMode.Right;
                } else if (_currentViewMode === _enumViewMode.Right) {
                    viewMode = _enumViewMode.Back;
                ...
                break;
            case _componentNames.ControlPointWest: // 前 - 左 - 后 - 右 - 前
                if (_currentViewMode === _enumViewMode.Front) {
                    viewMode = _enumViewMode.Left;
                } else if (_currentViewMode === _enumViewMode.Left) {
                    viewMode = _enumViewMode.Back;
    ...
    var isStandardView = function (dir) {
        ...
        // 两个为0，即为标准视图
        if ((isEquals(dirNorm.x, 0, _threshold) &&
            isEquals(dirNorm.y, 0, _threshold)) ||
            (isEquals(dirNorm.x, 0, _threshold) &&
            isEquals(dirNorm.z, 0, _threshold)) ||
            (isEquals(dirNorm.y, 0, _threshold) &&
            isEquals(dirNorm.z, 0, _threshold))) {
            return true;
        }
    ...
    var isBottomView = function (dir) {
        ...
        // Bottom (0, 1, 0)
        if (isEquals(dirNorm.x, 0, _threshold) &&
            isEquals(dirNorm.y, 1, _threshold) &&
            isEquals(dirNorm.z, 0, _threshold)) {
            return true;
        }
    ...
    var getCurrentStandardView = function (dir, up) {
        ...
        if (isEquals(dirNorm.x, 0, _threshold) &&
            ...
            // Top - up(0, 0, -1)
            if (isEquals(upNorm.x, 0, _threshold) &&
                isEquals(upNorm.y, 0, _threshold) &&
                isEquals(upNorm.z, -1, _threshold)) {
                return _enumViewMode.Top;
            }

            // TopTurnRight - up(-1, 0, 0)
            if (isEquals(upNorm.x, -1, _threshold) &&
                isEquals(upNorm.y, 0, _threshold) &&
                isEquals(upNorm.z, 0, _threshold)) {
        ...
        if (isEquals(dirNorm.x, 0, _threshold) &&
            isEquals(dirNorm.y, 0, _threshold) &&
            isEquals(dirNorm.z, -1, _threshold)) {

            // Front - up(0, 1, 0)
            if (isEquals(upNorm.x, 0, _threshold) &&
                isEquals(upNorm.y, 1, _threshold) &&
                isEquals(upNorm.z, 0, _threshold)) {
        ...
        // Right - dir(-1, 0, 0)
        if (isEquals(dirNorm.x, -1, _threshold) &&
            isEquals(dirNorm.y, 0, _threshold) &&
            isEquals(dirNorm.z, 0, _threshold)) {
    ...
    this.addDomEventListeners = function() {
        ...
        if (_houseContainer) {
            ...
            _houseContainer.addEventListener('contextmenu', function(event) { event.preventDefault(); }, false);
    ...
    this.removeDomEventListeners = function() {
        ...
        if (_houseContainer) {
            ...
            _houseContainer.removeEventListener('contextmenu', function(event) { event.preventDefault(); }, false);
    ...
    this.mouseUp = function (event) {
        ...
        if (_isRotate) {
            ...
            this.viewer.cameraEditor.processRotate(_rotateDelta);
    ...
    this.render = function () {
        ...
        if (!this.visible || this.isDisable)
            return;
        ...
        // 获得主场景的旋转角度
        var rotation = mainScene.getRotationGlobal();

        // 设置坐标轴的旋转向量
        _houseScene.rotation.copy(rotation);

        _houseCamera.up.copy(mainCamera.realUp || mainCamera.up);
        _houseCamera.lookAt(dir);
