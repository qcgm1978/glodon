...
(function () {
    var Assister = {
        ...
        isIos: function () {
            ...
            return !!osStr.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/); //ios终端
        },
        uploadSelected: function (CurrentClass, imageUrl, CurrentDetails) {
            setTimeout(function () {
            ...
            let length = $('.addImgBox').length;
            for (var i = 0; i < imageUrl.length; i++) {
                var fileName = imageUrl[i]["imageUrl"].substr(imageUrl[i]["imageUrl"].lastIndexOf('/') + 1);
                let dataIndex = length + i;
                $("#addImgList").append('<li class="addImgBox" name="' +
                    fileName +
                    '" data-index="' +
                    dataIndex +
                    '"><img src="data:image/jpeg;base64,' + imageUrl[i]["imageData"] + '" data-index="' +
                    dataIndex +
                    '" ' +
                    'data-width="' +
                    imageUrl[i].imageWidth +
                    '" ' +
                    'data-height="' +
                    imageUrl[i].imageHeight +
                        '"' +
                    '><span name="' +
                    fileName +
                    '" data-index="' +
                    ...
                    imageUrl[i].imageHeight +
                    '">上传中...</span></li>');
                CurrentDetails && CurrentDetails.defaults.imgSrcListData.push({
                    fileName: fileName, imgSrc: imageUrl[i]["imageData"]
                });
                CurrentClass.uploadmoreimage(imageUrl[i]["imageUrl"]);
                if ($('.addImgBox').length === 10) {
                    $('#addImgBtn').hide();
                    break;
                }
            }
