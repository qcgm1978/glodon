...
//                }
    ...
    var Upload = {
        ...
        config: function (sel) {
            ...
            url=url + (url.indexOf("?") == -1 ? "?" : "&") + "file="+me.editor.id;//初始form提交地址;
        ...
        uploadComplete: function(r){
            var me = this;
            var rs = r.substring((r.indexOf('>')+1),r.indexOf('</'));
            try{
                var json = eval("("+rs+")");
                Base.callback(me.editor, me.dialog, json.data.imageUrl, json.message);
            }catch (e){
    ...
    UM.registerWidget('image', {
        tpl: "<link rel=\"stylesheet\" type=\"text/css\" href=\"<%=image_url%>image.css\">" +
            ...
            "<div class=\"edui-image-dragTip\">点击上传</div>" +
            "</div>" +
