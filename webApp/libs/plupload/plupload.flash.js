		...
		init : function(uploader, callback) {
			...
			uploader.bind("Flash:Init", function() {				
				...
				uploader.bind("UploadFile", function(up, file) {
                    if (up.state === plupload.STOPPED) return;//add by baoym20130909
				...
				uploader.bind("Flash:UploadProcess", function(up, flash_file) {
					...
					if (file.status != plupload.FAILED) {
						//file.loaded = flash_file.loaded + (up.settings.uploaded_bytes || 0);
						file.loaded = Math.min(file.size, flash_file.loaded +(up.settings.uploaded_bytes || 0));
