        ...
        init: function (uploader, callback) {
            ...
            function addSelectedFiles(native_files) {
                ...
                for (i = 0; i < native_files.length; i++) {
                    ...
                    file.fullPath = file.fullPath || file.relativePath || file.webkitRelativePath;
                    if (fileNames[file.name + file.fullPath]) {
                        continue;
                    }
                    fileNames[file.name + file.fullPath] = true;
                    // Store away gears blob internally
                    id = plupload.guid();
                    html5files[id] = file;
                    // Expose id, name and size
                    files.push(new plupload.File(id, file.fileName || file.name, file.fileSize || file.size, file.fullPath)); // fileName / fileSize depricated
                }
            ...
            //export for page drag & drop use
            uploader.addFiles = addSelectedFiles;
            uploader.getNativeFiles = function () {
                return html5files;
            }
            ...
            uploader.bind("Init", function (up) {
                ...
                function initBrowseInput(inputId, triggerBrowseButton) {
                    ...
                    browseButton = document.getElementById(triggerBrowseButton);
                    if (browseButton) {
                ...
                initBrowseInput(uploader.id + '_html5', up.settings.browse_button);
            });
            // Add drop handler
            ...
            uploader.bind("UploadFile", function (up, file) {
                if (up.state === plupload.STOPPED) return;//add by baoym20130906
                ...
                function sendBinaryBlob(blob) {
                    ...
                    function uploadNextChunk() {
                        ...
                        function prepareAndSend(bin) {
                            ...
                            if (xhr.upload) {
                                xhr.upload.onprogress = function (e) {
                                    file.loaded = Math.min(file.size, loaded + e.loaded + up.settings.uploaded_bytes - multipartDeltaSize); // Loaded can be larger than file size due to multipart encoding
                            ...
                            xhr.onreadystatechange = function () {
                                ...
                                if (xhr.readyState == 4 && up.state !== plupload.STOPPED && file.status !== plupload.FAILED) {
                        ...
                        // Standard arguments
                        args = {name: file.target_name || file.name};
                        var uploadedBytes = settings.uploaded_bytes || 0;
                        ...
                        if (settings.chunk_size && file.size > settings.chunk_size && (features.chunks || typeof(blob) == 'string')) { // blob will be of type string if it was loaded in memory
                            ...
                            if (typeof(blob) == 'string') {
                                chunkBlob = blob.substring(chunk * chunkSize + uploadedBytes, chunk * chunkSize + curChunkSize);
                            } else {
                                // Slice the chunk
                                chunkBlob = w3cBlobSlice(blob, chunk * chunkSize + uploadedBytes, chunk * chunkSize + curChunkSize);
                            }
                            ...
                            //chunkBlob = blob;
                            if (typeof(blob) === 'string') {
                                chunkBlob = blob.substr(uploadedBytes);
                            } else {
                                // Slice the chunk
                                chunkBlob = w3cBlobSlice(blob, uploadedBytes, curChunkSize);
                            }
                        }
