...
CLOUD.MaterialSelector = function () {
    ...
    var _materialAttrs = [
        ...
        {name: 'highlight', param: {name: 'highlight', color: 0x1377C0, opacity: 1.0, transparent: false, side: THREE.DoubleSide}},      //liuw-d   由于高亮构件在优先显示是存在材质数据丢失问题采用此方式修改
...
CLOUD.Scene.prototype.getNearDepthByRect = function () {
    ...
    function calcNearDepth(object) {
        ...
        projectPosition.applyMatrix4(projectScreenMatrix);
...
CLOUD.Scene.prototype.LightPreset = function () {
    ...
    if (!this.ambientLight) {
        ...
        this.ambientLight = new THREE.AmbientLight(0xffffff, 0.40);
        this.add(this.ambientLight);
...
CLOUD.RectZoomTool.prototype.zoomToRectangle = function () {
    ...
    if (pivot) {
        ...
        console.log("Can't find intersect");
        var rcZoom = {};
        ...
        if (closeDepth !== Infinity){
            console.log("closeDepth :" + closeDepth);
...
CLOUD.Loader.ModelLoader.prototype._parseMpk = function (data) {
    ...
    for (var i = 0; i < count; ++i) {
        ...
        if ((normal == undefined) && (position !== undefined)) {    //liuw-d  兼容模型比较
            CLOUD.Logger.log("Error Geometry repair!");
            var nLength = position.length;
            normal = new Float32Array(nLength);
            for(var normalIndex = 0; normalIndex < nLength; ++normalIndex){
                var  remainder = normalIndex % 3;
...
CLOUD.Model.prototype._getMeshNodeAttribute = function (sceneOrSymbolReader, item, itemParent) {
    ...
    if (refMeshCache[meshId] === undefined) {    //liuw-d   兼容模型比较   此种应该不会再出现,此种异常由normal为空引起
...
CLOUD.ModelManager.prototype.updateSceneBoundingBox = function () {
    ...
    for (var id in models) {
        ...
        if (models.hasOwnProperty(id)) {
            ...
            if (boundingBoxTmp === null) {      //liuw-d    兼容模型比较
                continue;
            }                                   //liuw-d
