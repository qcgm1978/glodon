App.Projects.ContentMode = Backbone.View.extend({
	...
	showNullTip:function(){
		...
		var nullTxt = (App.Local.data["drawing-model"].Npe || '暂无可访问项目');
		if(language === "fr-FR"){
			nullTxt = (App.Local.data["drawing-model"].Npe || '暂无可访问项目');
		}else if(language === "en-US" || language === "en-GB"){
		   	nullTxt = 'No projects available.';
		...
		if (App.Projects.Settings.pageIndex!=1) {
			...
			this.$el.find(".proListBox").html('<li class="loading"><img src="/static/dist/images/projects/images/emptyProject.png"><div data-i18n="data.drawing-model.Npe">'+nullTxt+'</div></li>');
		}		
	}
