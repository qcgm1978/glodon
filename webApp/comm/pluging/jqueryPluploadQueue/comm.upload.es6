//@ sourceURL=comm_20160313.js
/*
* @require /comm/pluging/jqueryPluploadQueue/jquery.plupload.queue.es6
* */
/**
...
(function ($) {
    ...
    var upload = {
        init: function (container, options) {
            ...
            upload.versionId = options.versionId;
            upload.hasPrompt = options.hasPrompt;
            upload.isResource = options.isResource;
...
// debugger;
                    ...
                    if (items) {
                            ...
                            if (entry.isFile) {
                                ...
                                self.__traverseDirectory(entry, function (file) {
                                    if (options.isResource) {
                                        ...
                                        if (App.Project.Settings.CurrentVersion.subType !== 1) {
                                            allFiles.push(file)
                                        }
                                    }
                                })
                        ...
                        var fileTimer = setTimeout(function () {
                            if (allFiles.length === 0) {
                                !$('.plupload_filelist li').length && $(".mod-plupload").hide();
                                if (App.Project.Settings.CurrentVersion.subType === 1) {
                                    return alert('全标项目不能上传文件夹')
                                } else {
                                    return alert(App.Local.data['drawing-model'].EmptyFloder || '空文件夹不可上传');
                                }
                            }
        ...
        __initDirectoryUpload: function (options) {
            var chooseBtn = '#' + options.browse_button;
            var uploadButtons = $('<div class="upload-dropdown-buttons">' +
                '<ul>' +
                '   <li class="upload-file-btn" id="html5-uploadfile-btn"><em class="sicon-file"></em>&nbsp;&nbsp;' +
                (App.Local.data['project-files'].files || '文件') +
                '</li>' +
                '   <li class="upload-dir-btn" id="html5-uploaddir-btn"><em class="sicon-sfolder"></em>' +
                // '<a href="/static/dist/images/comm/pluging/jqueryPluploadQueue/img/target.png" data-lightbox="image-1" data-title="My caption">' +
                '&nbsp;&nbsp;' +
                (App.Local.data['project-dirFiles'].dirFiles || '文件夹') +
                // '</a>' +
                '</li>' +
                '</ul></div>')
