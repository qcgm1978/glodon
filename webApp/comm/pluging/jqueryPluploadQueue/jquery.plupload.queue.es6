...
(function ($) {
    ...
    $.fn.pluploadQueue = function (settings) {
        if (settings) {
            delete settings.browse_dir_button;
            this.each(function () {
                ...
                function changeStatus() {
                    var statusTitle,
                        statusText;
                    if (uploader.total.uploaded && uploader.total.uploaded === uploader.files.length && $('.mod-plupload').is(':visible')) {
                ...
                uploader.bind('FileUploaded', function (up, file) {
                    ...
                    handleStatus(file);
                    // debugger;
                    $('#' + file.id + ' div.pl-st').text(file.percent + '%');
                });
                uploader.bind("UploadProgress", function (up, file) {
                    //todo test code
                    // if(typeof uploadFirstProgress==='undefined'){
                    //     $('.plupload_filelist li:first a').click();
                    //     window.uploadFirstProgress=true;
                    //     return;
                    // }
                    // Set file specific progress
                    $('#' + file.id + ' div.pl-st', target).text(file.percent + '%');
                    if (file.hint === "上传成功" && file.percent !== 100) {
                        debugger;
                        file.status = 4;
                        var message = '上传失败';
                        $('#' + file.id).attr('class', 'plupload_failed clr').find('a').css('display', 'block').attr('title', message)
                            .end().find('.pl-st').text('0%');
                        // file.percent = 0;
                        // var actionClass = 'plupload_failed' + ' clr';
                        // var icon = $('#' + file.id).attr('class', actionClass).find('a');
                        // if (file.hint) {
