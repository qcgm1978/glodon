...
(function (root, factory) {
    ...
    var getDownloadUrl = function ($target) {
        if ($target.data("viewpointid")) {
            var projectid = parseInt(App.Project.Settings.projectId);
            var viewpointid = $target.data("viewpointid");
            ...
            var url = $target.closest("#lightbox").find("img.lb-image").prop("src");
            return url;
        }
    }
    ...
    Lightbox.prototype.enable = function () {
        ...
        $('body').on('click', 'a[rel^=lightbox], area[rel^=lightbox], a[data-lightbox], area[data-lightbox]', function (event) {
            if ($(event.target).parent().data("type") === 3) {
                ...
                App.Comm.previewFile({
                    ...
                    viewpointId: $(event.target).parent().data("viewpointid"),
                    attachmentId: $(event.target).parent().data("attachmentid"),
                    link: getDownloadUrl($(event.currentTarget))
                    // fail: function () {
                    //     self.start($(event.currentTarget));
                    // }
                }, $(event.target));
    ...
    Lightbox.prototype.build = function () {
        ...
        $('<div id="lightboxOverlay" class="lightboxOverlay"></div><div id="lightbox" class="lightbox"><div class="lb-outerContainer"><div class="lb-container"><img class="lb-image" src="data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==" /><div class="lb-nav"></div><div class="lb-loader"><a class="lb-cancel"></a></div></div></div><div class="lb-dataContainer"><div class="lb-data"><div class="lb-details"><span class="lb-caption"></span></div><div class="lb-closeContainer"><span class="downloadImg" data-i18n="data.drawing-model.Download">下载</span><i>|</i><span class="lookModel">' +
            (App.Local.data['drawing-model'].CMl || '查看模型') +
            '</span> <a class="lb-close"></a></div></div></div></div>').appendTo($('body'));
        ...
        this.$lightbox.find('.lb-loader, .lookModel').on('click', function (event) {//点击查看模型执行的方法
            ...
            var openurl = $(event.target).attr("data-openurl")
            if ($(event.target).html() == (App.Local.data['drawing-model'].CMl || '查看模型')) {
                App.Project.NotesCollection.clickModelHandle();
            } else if ($(event.target).html() == (App.Local.data['drawing-model'].CSMl || "查看单模型")) {
                window.open(openurl, '_blank');
            } else if ($(event.target).html() == (App.Local.data['drawing-model'].CDg || "查看图纸")) {
                window.open(openurl, '_blank');
            }
        });
        /*
    ...
    Lightbox.prototype.start = function ($link) {
        ...
        this.$lightbox.find(".lookModel").html((App.Local.data['drawing-model'].CMl || '查看模型'));
        this.$lightbox.find(".lookModel,i").show();
        this.$lightbox.find('.downloadImg').removeClass('downloadImg-right');
        if (imbox) {
            this.$lightbox.find(".lookModel").html("");
            this.$lightbox.find(".lookModel").siblings('i').hide();
            this.$lightbox
                .find('.downloadImg')
                .data("viewpointid", $link.data("viewpointid"))
                .data("attachmentid", $link.data("attachmentid"));
        } else if (type && type == 1) {
            this.$lightbox.find(".lookModel").html("");
            this.$lightbox
                .find('.downloadImg')
                .addClass('downloadImg-right')
                .data("viewpointid", $link.data("viewpointid"))
                .data("attachmentid", $link.data("attachmentid"));
            this.$lightbox.find(".lookModel,i").hide();
        } else if (notesType) {
            if (notesType == 1) {
                this.$lightbox.find(".lookModel").html((App.Local.data['drawing-model'].CSMl || "查看单模型")).attr("data-openurl", openurl);
            } else if (notesType == 2) {
                this.$lightbox.find(".lookModel").html((App.Local.data['drawing-model'].CDg || "查看图纸")).attr("data-openurl", openurl);
            }
        }
        this.$lightbox.css({
            top: top + 'px',
